...
- **Components in Markdown.** `::callout`, `::aside`, `::quote` and `::figure` are registered in `src/ui/components/prose/index.tsx` and listed in `tags.ts`. The parser validates documents against that list, so an unregistered `::name` fails the build instead of rendering as an unstyled inline element. Add a component to both files or not at all.
